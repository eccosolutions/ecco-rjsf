...
## Supported themes
 ...
 ---------- | ------- | -----------
 Bootstrap 3 (default) | Published | `@eccosolutions/rjsf-core`
...
first install both `@eccosolutions/rjsf-core` and `@rjsf/material-ui`. Then you can import the form by doing:
...
The default theme is bootstrap 3. In order to use another theme, you must first install `@eccosolutions/rjsf-core`.

For example, to use the standard bootstrap 3 form, you can run:

```js
import Form from "@eccosolutions/rjsf-core";
```

To use the material-ui form, you should first install both `@eccosolutions/rjsf-core` and `@rjsf/material-ui`. Then, you can run:
